...
import re
class Nic(object):
    def process(self,excucte_cmd,debug,host=None):
        if debug:
            output = open(os.path.join(settings.BASE_DIR, 'files/nic.out'), 'r', encoding='utf-8').read()
            interfaces_info = self._interfaces_ip(output)
        else:
            interfaces_info = self.linux_interfaces(excucte_cmd,host)

        self.standard(interfaces_info)

        return interfaces_info

    def linux_interfaces(self, command_func,host):
        '''
        ...
        ip_path = 'ip'
        if ip_path:
            cmd1 = command_func('sudo {0} link show'.format(ip_path),host)
            cmd2 = command_func('sudo {0} addr show'.format(ip_path),host)
